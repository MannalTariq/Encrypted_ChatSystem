...
// Function to pad input data to a multiple of AES_BLOCK_SIZE
static void pad_input(unsigned char *input, int *length) {
	int pad_len = AES_BLOCK_SIZE - (*length % AES_BLOCK_SIZE);
	for (int i = 0; i < pad_len; i++) {
		input[*length + i] = pad_len;
	}
	*length += pad_len;
}

// Function to encrypt a string using AES CBC mode
static void AES_EncryptText(AES_CTX *ctx, const unsigned char *key, const unsigned char *iv, const unsigned char *input, unsigned char *output) {
	int input_len = strlen((const char *)input);
	unsigned char padded_input[input_len + AES_BLOCK_SIZE];
	memcpy(padded_input, input, input_len);
	pad_input(padded_input, &input_len);

	AES_EncryptInit(ctx, key, iv);

	for (int i = 0; i < input_len; i += AES_BLOCK_SIZE) {
		AES_Encrypt(ctx, padded_input + i, output + i);
	}
}

// Function to decrypt a string using AES CBC mode
static void AES_DecryptText(AES_CTX *ctx, const unsigned char *key, const unsigned char *iv, const unsigned char *input, unsigned char *output) {
	int input_len = strlen((const char *)input);
